...
const team = () => {
  ...
  const [privileges, setPrivileges] = useState([
    ...
    {
      value: "View Course Participant and send email reminder",
      checked: false,
    },
    ...
    {
      value: "Switch Roles to Student platform of the Training Provider",
      checked: false,
    },
  ...
  const removeTeam = (id: any) => {
    apiService.delete(`/user/team/${id}/${user.id}`).then(function (response) {
  ...
  const updateStatus = (id: any, status: string) => {
    apiService
      .get(`/user/team/${user.id}/${id}/${status}`)
      .then(function (response) {
        console.log(response);
        api.open({
          message: response.data.message,
  ...
  const filteredTeam = team.filter(
    (single: any) =>
      single.ownerId?._id === user.id || single.status === "pending"
  );
      ...
      <main className="lg:flex justify-between px-6 py-4">
        ...
        <section className="lg:w-[48%] shadow-md rounded-md p-4 sm:mb-6">
          <div>
            ...
            {tutor && (
              <div className="my-4">
                <p className="mb-4">
                  Assign roles by selecting any of the following area:
                </p>

                {privileges.map((single, index) => (
        ...
        <section className="lg:w-[48%] shadow-md p-4 rounded-md">
          {filteredTeam.length >= 1 ? (
            filteredTeam.map((single: any) => (
              <div
                className="my-3 p-2 border-b border-[#808080] lg:flex justify-between"
                key={single._id}
              >
                <div className="flex cursor-pointer">
                  <img
                    className="h-14 w-14 mr-3 rounded-full my-auto object-cover"
                    src={
                      user.id === single.ownerId?._id
                        ? single.tutorId.profilePicture
                          ? single.tutorId.profilePicture
                          : "/images/user.png"
                        : single.ownerId.profilePicture || "/images/user.png"
                    }
                    alt=""
                  />
                  <div>
                    <p className=" capitalize text-lg">
                      {user.id === single.ownerId?._id
                        ? single.tutorId.fullname
                        : single.ownerId.fullname}
                    </p>
                    <p>
                      {user.id === single.ownerId?._id
                        ? single.tutorId.email
                        : single.ownerId.email}
                    </p>
                  </div>
                ...
                {open && (
                  <div>
                    ...
                    <div className="fixed top-10 bottom-10 left-0 overflow-y-auto rounded-md right-0 lg:w-[70%] w-[95%] mx-auto z-20 bg-[#F8F7F4]">
                      ...
                      <div className="p-4">
                        {privileges.map((single: any, index: any) => (
                          <div key={index} className="flex my-1">
                            <input
                              onChange={(e) =>
                                handleInputChange(index, e.target.checked)
                        ...
                        <button
                          className="p-4 bg-primary mt-6 px-6"
                          onClick={() => {
                            updatePriviledge(single.tutorId._id),
                              setOpen(false);
                          }}
                        >
                ...
                {user.id === single.ownerId?._id ? (
                  <div className="flex w-20 justify-between sm:mt-3">
                    <button
                      onClick={() => {
                        if (
                          isActionChecked("Edit team member", user.privileges)
                        ) {
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (
                          isActionChecked("Delete team member", user.privileges)
                        ) {
                  ...
                  <div className="flex w-38 justify-between">
                    <button
                      onClick={() =>
                        updateStatus(single.ownerId._id, "accepted")
                      }
                      className="bg-green-500 my-auto text-white px-2 py-1 rounded-full"
                    >
                      Accept
                    </button>
                    <button
                      onClick={() =>
                        updateStatus(single.ownerId._id, "rejected")
                      }
                      className="bg-red-500 my-auto px-2 text-white py-1 rounded-full"
                    >
                      Reject
                    </button>
                  </div>
            ...
            <div>
              ...
              <p className="text-center my-3">No added team member!</p>
            </div>
          )}
        </section>
      </main>
