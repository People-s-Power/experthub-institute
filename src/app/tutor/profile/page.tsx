...
const Profile = () => {
  ...
  const createRecipient = () => {
    ...
    try {
      ...
      apiService
        ...
        .catch((err) => {
          showNotification(
            err.response?.data || "Failed to update bank account",
            "error"
          );
        })
        .finally(() => {
  ...
  const addPic = () => {
    ...
    try {
      apiService
        .put(`user/updateProfilePicture/${user.id}`, formData)
        .then(function (response) {
          getUser();
  ...
  const addSign = () => {
    ...
    try {
      apiService
        .put(`user/signature/${user.id}`, formData)
        .then(function (response) {
          getUser();
  ...
  const updateUser = () => {
    ...
    try {
      ...
      apiService
        .put(`user/updateProfile/${user.id}`, {
          phone,
          gender,
          age,
          skillLevel: skill,
  ...
  const handleCancelPlan = () => {
    ...
    apiService
      .post(`transactions/cancel-premium/${user.id}`)
      .then((response) => {
      ...
      .catch((error) => {
        ...
        showNotification(
          error.response?.data?.message || "Failed to cancel plan",
          "error"
        );
      })
      .finally(() => {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {sign && sign.length > 0 && (
                    <button
                      onClick={() => {
                        if (
                          isActionChecked("Edit Signature", user.privileges)
                        ) {
                          addSign();
                        }
                      }}
        ...
        {notification && (
          <div
            className={`fixed top-4 right-4 z-[9999] p-4 min-w-[400px] rounded-md shadow-lg max-w-md transition-all duration-300 ${
